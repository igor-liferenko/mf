...
I've been playing around with using shared memory for Wayland buffers and didn't really like the idea of actually having to create a file on the file system soley to use as shared memory. Although I knew there's ways to deal with name-clashes, it still felt hacked together to me. Fournately, I found a better solution: the new memfd_create system call in Linux. This call never creates a file on the file system and has no name-clashes, everything is private. This blog post goes into more detail: https://dvdhrm.wordpress.com/tag/memfd/.
