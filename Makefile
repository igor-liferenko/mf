all:
	...
	@for i in `cd MFinputs/cm; grep -L Math cm*[0-9]*`; do g=`echo $$i | tr -dc 0-9`; sed "/font_identifier/s/CM/OM/;s/generate /&ld/;/generate/e printf 'def LHver_check(expr e,f)=enddef;\nebbase:=0;\nboolean roman_ec; roman_ec:=false;\ninput lcyrbeg;\ngensize:=%s;\ninput lhcodes;\ninput lcyrdefs;\ninput lxpseudo;\n' $$g" MFinputs/cm/$$i >MFinputs/om/om$${i#cm}; done # fikparm.mf equivalent (except that 'input lxpseudo' was moved from drivers)
