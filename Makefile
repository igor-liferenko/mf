all:
	...
	@for i in `cd MFinputs/om; ls om*`; do sed -n '/generate/w /dev/stdout' MFinputs/om/$$i | sed "s@generate \(\w*\).*@sed '/generate/r MFinputs/lh/\1.mf' MFinputs/om/$$i | sed '/generate/d;0,/.*lgrusu/s//%\&/;/%.*lgrusu/e grep -v endinput MFinputs/lh/lgrusu.mf' >MFinputs/om/$$(echo $${i%.*}|tr a-z A-Z).mf@"; done | sh # fonts to use with strut-based code
