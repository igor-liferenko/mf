all:
	...
	@for i in `cd MFinputs/cm; grep -L Math cm*[0-9]*`; do sed "/font_identifier/s/CM/OM/;s/generate /def LHver_check(expr e,f)=enddef;\nebbase:=0;\nboolean roman_ec; roman_ec:=false;\ninput lcyrbeg;\ngensize:=`echo $$i|tr -dc 0-9`;\ninput lhcodes;\ninput lcyrdefs;\n&ld/" MFinputs/cm/$$i >MFinputs/om/om$${i#cm}; done # equivalent to fikparm.mf # TODO: add ebbase and roman_ec in lh/README and remove from here
